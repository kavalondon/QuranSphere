    ...
    static func parseData() throws -> ([SurahMetadata], [JSONVerse]) {
        ...
        for (index, arVerse) in arabicResponse.verses.enumerated() {
            ...
            let verse = JSONVerse(
                ...
                // Fallbacks ensure safety: if Indopak is missing, it uses Uthmani. If both are missing, it shows a placeholder.
...
class LocalQuranManager: ObservableObject {
    ...
    private func loadQuranData() async {
        do {
            let (newSurahs, newVerses) = try await Task.detached {
                try await QuranDataParser.parseData()
            }.value
            
            await MainActor.run {
                self.surahs = newSurahs
                self.verses = newVerses
                print("✅ Successfully verified and loaded \(self.surahs.count) Surahs and \(self.verses.count) Verses directly from Quran.com!")
            }
        } catch {
